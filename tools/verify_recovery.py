...
ELITE_IN_RANGE = 0.512
ELITE_IN_RANGE_MIN = 0.40
# ── 보스전 (실측 · 하네스 24시드 중 도달 16판, 화력) ──────────────
...
BOSS_INFLOW     = 39.9    # /초. 맵 구간(22.8)보다 높다 — 보스 피해가 얹힌다
BOSS_ORB        = 12.9    # /초. 보스전에도 잡몹 스폰이 상한을 유지해 구슬은 계속 나온다
BOSS_LEECH      = 6.2     # /초. 회복 카드 몫 (화력 정책 기준)
BOSS_ARRIVE     = 323.0   # 도달 시 잠식 (최대치 1250)
BOSS_PHASE2_PURGE = _PROG["boss_phase2_purge"]

# **게이트는 평균 수지가 아니라 도달 후 생존율이다.**
# 한때 "회복 카드 포함 적자가 남은 게이지의 70~110%"로 뒀다가 접었다 — 평균
# 수지는 약한 빌드가 섞이는 정도에 끌려다닌다. 맵이 쉬워져 보스 도달률이
# 15/24 → 16/24로 오르자 같은 설정에서 98% → 139%로 튀어 밴드를 벗어났다.
# 게임이 나빠진 게 아니라 **모집단이 바뀐 것**이었다.
#
...
BOSS_SURVIVE_MEASURED = 0.327   # 실측 (하네스 100시드 × 정책 4종)
...
MEASURED = [
    ...
    ( 14.6,  31.6),   # 2 — 물량 임계 20 돌파
    ( 22.2,  34.3),   # 3 — 엘리트가 닿기 시작한다
    ( 23.5,  37.4),   # 4
    ( 25.9,  36.7),   # 5
    ( 27.5,  50.6),   # 6
    ( 26.9,  61.7),   # 7
...
def report():
    ...
    print("     유무도 가른다 (실측: 0 → 17.5% · 400 → 32.7%)")
